LinearSaw = class extends Trap
{
	...
	update()
	{
		...
		if(this.bladeRoute>=180)this.bladeDirection=-1;
		if(this.bladeRoute<=1)this.bladeDirection=1;
		var xMod,yMod;
		if(!this.blade.spinner)
		{
			xMod = Math.cos(this.blade.rotation);
			yMod = Math.sin(this.blade.rotation);
		}
		else
		{
			xMod = Math.cos(this.blade.rotation+this.blade.initialRotation);
			yMod = Math.sin(this.blade.rotation+this.blade.initialRotation);
		}
